...
from azeuserbot-install import *
...
def createApp (connect):
    appname = "Aze" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
async def botlog (String, Api, Hash):
    ...
    KanalId = await Client(CreateChannelRequest(
        title='AzeUserBot BotLog',
        about=LANG['AUTO_BOTLOG'],
        megagroup=True
    ))
    KanalId = KanalId.chats[0].id

    Photo = await Client.upload_file(file='azeuserbotlogo.jpg')
...
if __name__ == "__main__":
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/vusal0868/azeuserbot"
    config['WARN_LIMIT'] = "3"
